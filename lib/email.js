...
export async function sendRegistrationEmail(registration) {
    ...
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="border:1px solid #e7e2d6;border-top:none;border-radius:0 0 8px 8px;padding:8px 0;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          ...
          ${row("Appointment Time", registration.appointmentSlotLabel || "N/A")}
          ${row("Late Policy Acknowledged", registration.hairService ? (registration.ackLatePolicy ? "Yes" : "No") : "N/A")}
  ...
  const text = [
    ...
    `Appointment Time: ${registration.appointmentSlotLabel || "N/A"}`,
    `Late Policy Acknowledged: ${registration.hairService ? (registration.ackLatePolicy ? "Yes" : "No") : "N/A"}`,
  ...
  return resend.emails.send({
    ...
    // The Resend SDK does NOT always throw on failure — invalid API keys,
    // unverified domains, and sandbox-sender restrictions often come back
    // as a quiet { error } field instead of a thrown exception. Without this
    // check, a failed send would look identical to a successful one.
    if (result?.error) {
      throw new Error(
        `Resend rejected the email: ${result.error.message || JSON.stringify(result.error)}`
      );
    }
    return result;
  });
